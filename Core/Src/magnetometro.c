...
void HMC5883L_Init(I2C_HandleTypeDef *hi2c){
    ...
    HAL_I2C_Mem_Write(&hi2c, HMC5883L_ADDRESS, REG_CONFIG_A, 1, &configA, 1, HAL_MAX_DELAY);
...
void HMC5883L_ReadData(I2C_HandleTypeDef *hi2c, XYCoordinates *reading[8]){
    ...
    for (int i = 0; i < 8; i++) {
        ...
        HAL_I2C_Mem_Read(&hi2c, HMC5883L_ADDRESS << 1, REG_DATA_OUT_X_MSB, 1, rawData, 6, HAL_MAX_DELAY);
...
uint8_t Filter_Data(XYCoordinates *reading[8], XYCoordinates *result[8]){
...
XYCoordinates Data_Means(XYCoordinates reading[8], uint8_t n){
	...
	XYCoordinates coordinateMeans = {meansX/8, meansY/8};
...
float XY_to_Degrees(XYCoordinates *coordinate){
	float angle = atan2(coordinate->y, coordinate->x)*(180.0/M_PI);
